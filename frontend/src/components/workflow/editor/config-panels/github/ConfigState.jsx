...
import { ChevronRight, LogOut, Plus, Save, X } from 'lucide-react';
...
const ConfigState = ({ selectedNode, setNodeConfig, onClose, data, handleConnect }) => {
  ...
  const {
    ...
    defaultValues: {
      ...
      actionOrEvent:
        existingConfig.actionOrEvent ||
        (isTriggerNode ? GITHUB_TRIGGER_OPTIONS[0].value : GITHUB_ACTION_OPTIONS[0].value),
  ...
  const selectedActionOrEvent = watch('actionOrEvent');

  const selectedAccount = useMemo(() => {
    if (!Array.isArray(data) || data.length === 0) return null;
    return data.find((account) => account.external_id === existingConfig.githubAccountId) || data[0];
  }, [data, existingConfig.githubAccountId]);
  ...
  const onSubmit = async (formData) => {
    const payload = {
      ...
      event: isTriggerNode ? formData.actionOrEvent : '',
      action: isTriggerNode ? '' : formData.actionOrEvent,
    };
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex-1 flex flex-col overflow-hidden">
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-6">
        <section className="space-y-3 rounded-xl border border-zinc-800 bg-zinc-900/30 p-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold uppercase tracking-wide text-zinc-300">Authentication</h3>
            <button
              type="button"
              onClick={handleConnect}
              className="inline-flex items-center gap-2 rounded-lg border border-zinc-700 px-3 py-1.5 text-xs font-medium text-zinc-100 hover:bg-zinc-800"
            >
              <Plus size={13} /> Connect GitHub Account
            </button>
          </div>
        ...
        <section className="space-y-3">
          ...
          <div className="relative">
            <select {...register('actionOrEvent', { required: true })} className={selectClass}>
        ...
        {selectedActionOrEvent.startsWith('pull_request') && isTriggerNode && (
          <section className="space-y-3">
        ...
        {selectedActionOrEvent === 'create_issue' && (
          <section className="space-y-3 rounded-xl border border-zinc-800 bg-zinc-900/30 p-4">
            <label className="text-sm font-bold uppercase tracking-wider text-zinc-200">Title</label>
            <input
              {...register('title', {
                validate: (value) =>
                  selectedActionOrEvent !== 'create_issue' || value.trim() ? true : 'Title is required.',
              })}
        ...
        {selectedActionOrEvent === 'comment_on_issue' && (
          <section className="space-y-3 rounded-xl border border-zinc-800 bg-zinc-900/30 p-4">
            ...
            <textarea
              {...register('comment', {
                validate: (value) =>
                  selectedActionOrEvent !== 'comment_on_issue' || value.trim() ? true : 'Comment is required.',
              })}
              rows={5}
        ...
        {selectedActionOrEvent === 'create_pull_request' && (
          <section className="space-y-3 rounded-xl border border-zinc-800 bg-zinc-900/30 p-4">
            <label className="text-sm font-bold uppercase tracking-wider text-zinc-200">Title</label>
            <input
              {...register('title', {
                validate: (value) =>
                  selectedActionOrEvent !== 'create_pull_request' || value.trim()
                    ? true
                    : 'Title is required.',
              })}
        ...
        {['create_file', 'update_file'].includes(selectedActionOrEvent) && (
          <section className="space-y-3 rounded-xl border border-zinc-800 bg-zinc-900/30 p-4">
            <label className="text-sm font-bold uppercase tracking-wider text-zinc-200">File Path</label>
            <input
              {...register('filePath', {
                validate: (value) =>
                  !['create_file', 'update_file'].includes(selectedActionOrEvent) || value.trim()
                    ? true
      ...
      <div className="border-t border-zinc-700 px-4 py-3 bg-zinc-900/40 flex items-center justify-end gap-3">
        <button
          type="button"
          onClick={onClose}
