...
const GoogleFormConfig2 = ({ selectedNode, onClose, setNodeConfig }) => {
  const { workflowId } = useWorkflowData();
  const { data, isLoading, isError, isFetching, refetch } = useGetIntegration('googleForm');

  const handleConnect = () => {
    window.open(
      `http://localhost:5000/api/integration/oauth/googleForm/connect?workflowId=${workflowId}`,
      '_blank'
    );
  };
        ...
        <div>
          <h2 className="text-3xl font-semibold text-zinc-100 font-mono flex items-center gap-3">
            <span className="p-2 border border-zinc-400 rounded-xl">
              <img src="/googleform.svg" alt="Google Form" className="h-6 w-6" />
            </span>{' '}
            Google Form
          </h2>
          <p className="text-sm text-[#E5E5E5] mt-2 font-normal">
            Configure trigger and actions for your Google Form workflows.
          </p>
        </div>
        <CloseBtn onClose={onClose} />
      </div>
...
export default GoogleFormConfig2;
