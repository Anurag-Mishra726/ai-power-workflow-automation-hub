...
const GmailConfigState = ({ selectedNode, onClose, setNodeConfig }) => {
  const { workflowId } = useWorkflowData();
  const { data, isLoading, isError, isFetching, refetch } = useGetIntegration('gmail');
  ...
  const handleConnect = () => {
    window.open(
      `http://localhost:5000/api/integration/oauth/gmail/connect?workflowId=${workflowId}`,
      '_blank'
    );
  };
      ...
      <div className="flex px-4 py-3 border-b-2 border-zinc-700 relative">
        <div>
          ...
          <p className="text-sm text-[#E5E5E5] mt-2 font-normal">
            Configure trigger and actions for your Gmail workflows.
          </p>
        </div>
...
export default GmailConfigState;
