...
export const handleCommentOnIssue = async ({ data, nodeId, context, userId }) => {
  ...
  const issueNumber = Number(render(config.issueNumber, context));
  const body = render(config.comment || config.commentMessage, context);
  const installationId = config.githubAccountId;
  const startTime = Date.now();

  if (!installationId || !owner || !repo || !issueNumber || !body) {
